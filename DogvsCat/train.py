...
parser.add_argument('--summary_title', default='VGG11 with BCELoss')
...
train_loader, validation_loader = get_loader(args.data_dir, args.image_size, args.batch_size)
...
for iter in range(args.epoch, args.num_epoch):
    print(iter)
    train_loss, train_acc = net.train_model(train_loader, criterion, optimizer)
    validation_loss, validation_acc = net.validate_model(validation_loader, criterion)
    writer.add_scalars(f'{args.summary_title}/Loss', {'train': train_loss, 'val': validation_loss}, iter)
    writer.add_scalars(f'{args.summary_title}/Accuracy', {'train': train_acc, 'val': validation_acc}, iter)
